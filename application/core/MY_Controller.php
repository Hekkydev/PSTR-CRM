...
class MY_Controller extends CI_Controller {
    ...
    function __construct() {
        ...
        // ======== Load Library ========
        $this->load->library(array('session','form_validation','pagination','ion_auth','general','reservation'));

        $this->load->config('ci-blog');
        $this->link_site_front = $this->config->item('ci_url_website');
        ...
        if(empty($this->data['page_title'])){
            ...
            $this->title_site = $this->config->item('ci_blog_sitename').' - '.$this->config->item('ci_blog_tagline');
        }
    ...
    protected function title_page($title)
    {
        return $this->title_page = $title;
    }

    protected function load_theme($content = null, $layout = true) {
        $this->load->model('Menu');
        $this->data['main_menus'] = '';
        $this->data['side_menus'] = '';
       
        if(count($this->Menu->findActive()) > 0){
            $this->data['main_menus'] = $this->general->bootstrap_menu($this->Menu->findActive_order());
            $this->data['side_menus'] = $this->general->side_menus($this->Menu->findActive_order());
        }
    ...
    public function load_theme_post($content,$data = null)
    {
        $this->load->model('Menu');
        $this->data['main_menus']   = '';
        $this->data['side_menus']   = '';
        if(count($this->Menu->findActive()) > 0){
        $data['main_menus']         = $this->general->bootstrap_menu($this->Menu->findActive_order());
        $data['side_menus']         = $this->general->side_menus($this->Menu->findActive_order());
        }
        $base_assets_url            = 'assets/'.THEMES_DIR.'/'.$this->config->item('ci_blog_theme').'/';
        $data['base_assets_url']    = BASE_URI.$base_assets_url;
        $data['title_site']         = $this->title_site;
        $data['content'] = (is_null($content)) ? '' : $this->load->view(THEMES_DIR.'/'.$this->config->item('ci_blog_theme').'/'.$content, $this->data, TRUE);
        $this->load->view(THEMES_DIR.'/pasteurtrans/content_post',$data);
    }

    public function load_theme_booking($content,$data = null)
    {
        $this->load->model('Menu');
        $this->data['main_menus']   = '';
        $this->data['side_menus']   = '';
        ...
        $base_assets_url            = 'assets/'.THEMES_DIR.'/'.$this->config->item('ci_blog_theme').'/';
        $this->data['base_assets_url']    = BASE_URI.$base_assets_url;
        $data['title_site']         = $this->title_site;
        $data['content'] = (is_null($content)) ? '' : $this->load->view(THEMES_DIR.'/'.$this->config->item('ci_blog_theme').'/'.$content, $this->data, TRUE);
        $this->load->view(THEMES_DIR.'/pasteurtrans/content_booking',$data);
    ...
    protected function load_admin($content = null, $layout = true) {
    	...
    	if($layout == true){
            ...
            $this->data['link_site_url'] = $this->link_site_front;
